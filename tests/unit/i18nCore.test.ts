/**
 * Locale layer core proofs (D42, 02 section 16): resolution order (embed
...
describe('resolveLocale', () => {
  it('prefers the embed attribute over tenant config and navigator', () => {
    expect(
      resolveLocale({ embedLocale: 'es-MX', tenantLocale: 'fr', navigatorLanguage: 'de' }),
    ).toBe('es-MX');
  });
  ...
  it('falls back to tenant config when the embed attribute is absent or invalid', () => {
    ...
    expect(
      resolveLocale({ embedLocale: '!!not-a-locale!!', tenantLocale: 'fr-CA', navigatorLanguage: 'de' }),
    ).toBe('fr-CA');
...
describe('createIntlFormatters', () => {
  ...
  it('formats dates, times, numbers, currencies, and relative times', () => {
    ...
    expect(formatters.formatDate(date)).toBe(
      new Intl.DateTimeFormat('en-US', { dateStyle: 'medium' }).format(date),
    );
    expect(formatters.formatTime(date)).toBe(
  ...
  it('accepts timestamps and rejects invalid dates', () => {
    ...
    expect(formatters.formatDate(timestamp)).toBe(
...
describe('logical properties guidance (D42 layout contract)', () => {
