/**
 * D56 unit coverage: the declared virtualization threshold, the pure
...
describe('declared threshold (D56)', () => {
  ...
  it('windows only above the threshold, not at it', () => {
    expect(shouldVirtualize(LIST_VIRTUALIZATION_THRESHOLD, LIST_VIRTUALIZATION_THRESHOLD)).toBe(
      false,
    );
    expect(
      shouldVirtualize(LIST_VIRTUALIZATION_THRESHOLD + 1, LIST_VIRTUALIZATION_THRESHOLD),
    ).toBe(true);
...
describe('computeVirtualWindow', () => {
  ...
  it('starts at the top with no top spacer', () => {
    ...
    expect(window.end).toBeLessThanOrEqual(
      maxWindowRowCount(base.viewportHeightPx, base.rowHeightPx, base.overscanRows),
    );
  ...
  it('keeps spacers plus window rows summing to the full scroll height', () => {
    for (const scrollTop of [0, 37, 400, 1999, 4280, 999999]) {
      ...
      expect(windowRows).toBeLessThanOrEqual(
        maxWindowRowCount(base.viewportHeightPx, base.rowHeightPx, base.overscanRows),
      );
    }
  });
...
describe('extractListRows', () => {
  ...
  it('maps template fields, bare and $row-prefixed, onto rows', () => {
    ...
    const prefixed = extractListRows(
      items,
