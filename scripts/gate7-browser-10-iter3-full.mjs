/**
 * wave-3 — 10-locale-rtl browser verification (styling remediation).
 */
...
const cacheBust = 'iter3-gallery-dark-chrome-20260723';
...
const report = {
  ...
  remediation:
    'Full panel-chrome base styles in panel-embed-gallery-dark.css; panel-band edge contrast; gallery-dark surface tokens',
  consoleMessages: [],
  consoleWarnings: [],
  consoleErrors: [],
  captures: [],
...
function collectMetrics {
  ...
  const chromeBtn = shadow?.querySelector('.panel-chrome__btn');
  const surface = shadow?.querySelector('.agentable-panel-surface');
  const chromeStyles = chromeBtn ? getComputedStyle(chromeBtn): null;
  const surfaceStyles = surface ? getComputedStyle(surface): null;
  const list = shadow?.querySelector('agentable-virtual-list');
  const text = list?.shadowRoot?.textContent ?? shadow?.textContent ?? '';
  return {
    ...
    panelDir: shadow?.querySelector('[dir]')?.getAttribute('dir') ?? null,
    ...
    chromeBtnWidth: chromeStyles?.width ?? null,
    chromeBtnHeight: chromeStyles?.height ?? null,
    chromeBtnDisplay: chromeStyles?.display ?? null,
    surfaceBackground: surfaceStyles?.backgroundColor ?? null,
    surfaceBorderColor: surfaceStyles?.borderColor ?? null,
  };
}
...
try {
  ...
  const chromeStyled =
    ...
    (report.metrics.desktop?.chromeBtnDisplay === 'inline-flex' ||
      report.metrics.desktop?.chromeBtnDisplay === 'flex');
