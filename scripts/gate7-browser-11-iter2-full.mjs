/**
 * — 11-app-shell QA (styling + resize + regression).
 * Playwright supplementary when cursor-ide-browser MCP ghost tabs block capture.
 */
...
const cacheBust = 'iter2-qa-20260723';
...
const report = {
  ...
  cursorIdeBrowserError:
    'browser_tabs new -> view not found; browser_navigate -> No browser tab available (ghost tab)',
  build: 'npm run build:embed:app-shell (2026-07-23)',
  consoleErrors: [],
  captures: [],
  steps: [],
  metrics: {},
...
try {
  ...
  report.steps.push({ step: 'R1', action: 'split default', ok: true, sidebarWidthPx: defaultWidth });
  await captureShell(page, 'cursor-browser-r01-split-default.png', 'R1');
  ...
  report.stylingChecks = {
    ...
    noLightFixture: !documentLightFixture(d),
  ...
  if (!report.resizeChecks.sidebarWidthChanged) resizeFail.push('sidebar width did not change on drag');
  if (!report.resizeChecks.noHorizontalScrollExplosion) resizeFail.push('horizontal scroll explosion');
  if (!report.resizeChecks.bothPanelsReadable) resizeFail.push('panels not readable after resize');
...
function documentLightFixture(metrics) {
  return metrics.bodyBackground === 'rgb(248, 250, 252)';
}
