/**
 * Static server for embed + gallery e2e.
...
function ensureGalleryAssetsBuilt {
  run(process.platform === 'win32' ? 'npx.cmd': 'npx', [
    ...
    'build',
    '--config',
    'vite.gallery-harnesses.config.ts',
  ]);
  run(process.platform === 'win32' ? 'npx.cmd': 'npx', [
    'vite',
    'build',
    '--config',
    'vite.e2e-multi-agent.config.ts',
  ]);
}
...
server.listen(port, '127.0.0.1', () => {
  process.stdout.write(`e2e gallery server listening on http://127.0.0.1:${port}\n`);
});
