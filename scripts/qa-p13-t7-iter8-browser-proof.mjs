/**
 * browser proof — honest gates for Build/Draw tool cards.
 * Run: node scripts/qa-p13-t7-iter8-browser-proof.mjs
 */
...
async function countCanvasShapes(page) {
  return page.evaluate(async () => {
    ...
    if (read.ok && read.result && typeof read.result === 'object' && read.result !== null) {
      const graph = /** @type {{ shapes?: Array<{ nativeType?: string; props?: Record<string, unknown> }> }} */ (
        read.result
      );
      const shapes = graph.shapes ?? [];
      const blueGeo = shapes.filter((shape) => {
        if (shape.nativeType !== 'geo') {
          return false;
        }
        const fill = shape.props?.fill;
        const color = shape.props?.color;
        return fill === 'solid' && (color === 'blue' || color === '#0000ff');
      }).length;
...
async function main {
  ...
  proofs.push({
    ...
    pass:
      ...
      (shapeCountIncreased || hasBlueGeo || hasCreatedShapeIds) &&
      (/draw_shapes|rectangle|drew/i.test(drawText) || hasCreatedShapeIds),
