...
const heartProbe = await page.evaluate(async () => {
  ...
  return {
    ...
    hasLegacyHeartIds: allIds.some((id) =>

      ['heart-lobe-left', 'heart-lobe-right', 'heart-point'].includes(id)),
...
const scan = {
  ...
  semanticFidelity: {
    ...
    heartNotSketchABC: heartProbe.hasSketchABC === false && heartProbe.hasHeartOutline === true,

    heartLabelOk: /heart sketch/i.test(String(heartProbe.assistantText)),

    singleShapeOutline: heartProbe.shapeIds.filter((id) => id.includes('heart')).length === 1,

  },

  visualQualityNote:

    'Single closed freehand heart-outline with light-red solid fill. Playwright PNG submitted for qa-expert visual gate — do not claim PASS here.',
...

    {
      ...
      entries: [

        {
          ...
          purpose: 'SC5 heart visual recognizability (iter-19 closed silhouette)',
