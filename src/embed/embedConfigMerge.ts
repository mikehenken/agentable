...
function resolveToolbarConfig(
  ...
  doc: EmbedConfigDocument,
): WhiteboardToolbarConfig | undefined {
...
export function mergeEmbedConfig(
  ...
  const greetingModeRaw = pickString(
    ...
    docPersona.greetingMode !== undefined
      ? String(docPersona.greetingMode)
      : doc.greetingMode !== undefined
        ? String(doc.greetingMode)
        : undefined,
  ...
  const fullpageOnEngage = attrs.fullpageOnEngageSet
    ? attrs.fullpageOnEngage || attrs.fullscreenOnEngage
    : engageFromDoc === true
      ? true
      : defaults.fullpageOnEngage;
  ...
  const tenantConfig: PartialCanvasTenantConfig = {
    ...
    persona: {
      ...
      ...(docPersona.assistantName ? { assistantName: docPersona.assistantName } : {}),
      ...(docPersona.tenantTitle ? { tenantTitle: docPersona.tenantTitle } : {}),
      ...(docPersona.starterPrompts ? { starterPrompts: docPersona.starterPrompts } : {}),
      ...(docPersona.geminiVoiceName ? { geminiVoiceName: docPersona.geminiVoiceName } : {}),
      ...(docPersona.chatProxyUrl ? { chatProxyUrl: docPersona.chatProxyUrl } : {}),
      ...(docPersona.mockScenario ? { mockScenario: docPersona.mockScenario } : {}),
      ...(personaVisual ? { visual: personaVisual } : {}),
    },
  ...
  warnVoiceGreetingConfig(tenantConfig.persona ?? {}, {
    tenant,
    rawGreetingMode: greetingModeRaw.trim()
      ? greetingModeRaw
      : docPersona.greetingMode ?? doc.greetingMode,
  });
