...
import { GRID_REFERENCE_WIDTH } from '../../canvas/gridLayout';
...
export function hasSiteContextPanels(editor: Editor, siteId: string): boolean {
...
export interface SiteContextArrangeOptions {
...
export function autoArrangeSiteContextPanels(
  ...
  reflowSiteContextRow(editor, frameId);
  return true;
}
...
export function autoArrangeSiteContextPanelsByFrameId(
  ...
  return autoArrangeSiteContextPanels(editor, meta.id);
}

/** Auto-arrange every site context frame on the current page. */
export function autoArrangeAllSiteContextPanels(editor: Editor): number {
  let arranged = 0;
  for (const shape of editor.getCurrentPageShapes()) {
    if (shape.type !== 'frame') continue;
    const meta = getContextGroupMeta(shape);
    if (meta?.kind !== 'site') continue;
    if (autoArrangeSiteContextPanels(editor, meta.id)) {
      arranged += 1;
    }
  }
  return arranged;
