...
import { GRID_SIZE, type LayoutRect } from '../../canvas/panelLayoutEngine';
import { GRID_GUTTER } from '../../canvas/gridLayout';
import {
  CONTEXT_FRAME_PADDING,
  findSiteContextGroupForShape,
  getContextGroupMeta,
} from './contextGroupApi';
import type { SiteContextPanelKind } from './siteContextPanelLayout';
...
export function previewPanelDockHighlight(
  ...
  const ctx = findSiteContextGroupForShape(editor, draggedShapeId);
  if (!ctx) return null;
...
/** True while `reflowSiteContextRow` is applying panel geometry. */
 ...
 */
export function reflowSiteContextRow(editor: Editor, frameId: TLShapeId): void {
  const frame = editor.getShape(frameId);
...
function sortDockNodesByDependency(nodes: DockTreeNode[]): DockTreeNode[] {
  ...
  const placed = new Set<SiteContextPanelKind>();
...
function editorShapeIdToPanelId(shapeId: TLShapeId): SiteContextPanelKind | null {
