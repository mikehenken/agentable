...
export function isPanelInsideSiteContextFrame(editor: Editor, shapeId: TLShapeId): boolean {
...
export function ejectGlobalPanelsFromSiteFrames(editor: Editor): number {
  ...
  for (const panelId of CANVAS_GLOBAL_PANEL_IDS) {
    ...
    if (!isPanelInsideSiteContextFrame(editor, shapeId)) continue;
...
export function filterSiteContextPanelIds(panelIds: string[]): string[] {
