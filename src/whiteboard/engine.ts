...
import type { JsonObject, SpecOrigin } from '../panels/types';
import {
  closePanelInCanvas,
  getEditor,
...
function recordOrigin(data: Record<string, unknown>): SpecOrigin {
  return data.origin === 'agent' ? 'agent' : 'host';
}

function applyCanvasMode(editor: Editor, mode: CanvasMode): void {
  switch (mode.kind) {
    case 'infinite':
    ...
    case 'bounded': {
      ...
      const zoomOptions =
        zoom === 'locked'
          ? { wheelBehavior: 'pan' as const, zoomSteps: [editor.getZoomLevel()] }
          : zoom !== undefined
            ? { wheelBehavior: 'zoom' as const, zoomSteps: buildZoomSteps(zoom) }
...
export function createWhiteboardEngine(): WhiteboardEngineHandle {
  ...
  const handleStoreEntry = (
    bound: Editor,
    changes: HistoryEntry<TLRecord>['changes'],
  ): void => {
  ...
  return {
    ...
    setZOrder(id: PanelInstanceId, z: 'front' | 'back' | number): void {
      ...
      const panels = bound
        ...
        .map(asPanelShape)
        .filter((panel): panel is PanelShapeRecord => panel !== null)
        .filter((panel) => panel.id !== shapeId)
    ...
    importLayout(records: WorkspaceLayoutRecord[]): void {
      for (const record of records) {
        openPanelInCanvas(record.panelId, {
          ...
          panelProps: {
            ...(record.contextId !== null ? { contextRef: record.contextId } : {}),
            ...(record.origin === 'agent' ? { origin: 'agent' } : {}),
          },
        });
      }
    },
    ...
    capabilities: WHITEBOARD_ENGINE_CAPABILITIES,
