  ...
  switch (name) {
    case 'draw_shapes': {
      ...
      if (
        ...
        const layoutLabel =
          layout === 'flow'
            ? 'flow diagram': layout === 'timeline'
              ? 'timeline': layout === 'radial'
                ? 'radial map': 'architecture diagram';
        if (ok) {
          return nodeCount !== undefined
            ? `Drew ${layoutLabel} · ${nodeCount} nodes`: `Drew ${layoutLabel}`;
        }
      ...
      if (shapeCount !== undefined) {
        return ok
          ? `Drew ${shapeCount} shape${shapeCount === 1 ? '': 's'} on canvas`: toolError ?? 'Draw shapes failed';
      }
