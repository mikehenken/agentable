...
import { autoArrangeContextFramePanels } from './contextFrameAutoArrange';
...
export interface ContextFrameWorkspaceModeOptions {
 ...
 */
export function enterContextFrameWorkspaceMode(
  editor: Editor,
  siteId: string,
  options: ContextFrameWorkspaceModeOptions = {}): boolean {
  const {
    arrangeFirst = true,
  ...
  if (arrangeFirst) {
    autoArrangeContextFramePanels(editor, siteId);
  }
