...
import type { ContextFramePanelKind, ContextFramePanelPlacement } from './contextFramePanelLayout';



export interface ContextFrameDockPresetOptions {
...
function dockToPanelRight(panelId: ContextFramePanelKind): PanelDock {
...
export function buildAdminSiteDockTree(
  ...
  options: ContextFrameDockPresetOptions,

  sizes: Map<ContextFramePanelKind, { w: number; h: number }>): DockTreeNode[] {
  ...
  if (includeBrief) {
    ...
    if (size) {
      ...
      const dock: PanelDock = includeChat
        ...
        ? dockToPanelRight('chat'): dockToGroupLeft(frameId);
  ...
  if (includePreview) {
    ...
    if (size) {
      ...
      const anchor: ContextFramePanelKind = includeBrief

        ? 'project-brief': includeChat
      ...
      const dock: PanelDock =
        ...
        anchor === 'web-preview'
          ...
          ? dockToGroupLeft(frameId): dockToPanelRight(anchor);
...
export function sizesFromPlacements(
  ...
  placements: ContextFramePanelPlacement[]): Map<ContextFramePanelKind, { w: number; h: number }> {

  const map = new Map<ContextFramePanelKind, { w: number; h: number }>();
