...
/** Canonical meta key on tldraw context-frame shapes (A10). */
export const CONTEXT_META_KEY = 'agentableContextFrame';

/** Legacy meta key — read during the one-minor migration window. */
export const LEGACY_CONTEXT_META_KEY = 'landiContextGroup';
...
export interface ResolvedContextFrameGroup {
  siteId: string;
  frameId: TLShapeId;
  label: string;
}
 ...
 */
export function resolveContextFrameFromSelection(editor: Editor): ResolvedContextFrameGroup | null {
  const selectedIds = editor.getSelectedShapeIds();
  if (selectedIds.length === 0) return null;

  const siteIds = new Set<string>();
  let resolved: ResolvedContextFrameGroup | null = null;
...
export function fitContextFrameGroupForShape(
  ...
  if (shape?.type === 'panel') {
    ensurePanelInContextFrame(editor, shapeId, ctx.frameId);
  }
