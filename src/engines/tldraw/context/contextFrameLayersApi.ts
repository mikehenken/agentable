...
export interface ContextFrameLayer {
  shapeId: TLShapeId;
  name: string;
  panelId: string | null;
  visible: boolean;
...
export function isContextFrameLayerVisible(shape: TLShape): boolean {
 ...
 */
export function listContextFrameLayers(editor: Editor, siteId: string): ContextFrameLayer[] {
  ...
  const layers: ContextFrameLayer[] = [];
  ...
  for (const shapeId of childIds) {
    ...
    layers.push({
      ...
      visible: isContextFrameLayerVisible(shape),
    });
  }
 ...
 */
export function resolveSelectedContextFrameLayerId(
  editor: Editor,
...
export function selectContextFrameLayer(
  editor: Editor,
  shapeId: TLShapeId,
  siteId: string): boolean {
  const isLayer = listContextFrameLayers(editor, siteId).some(
...
export function toggleContextFrameLayerVisibility(editor: Editor, shapeId: TLShapeId): boolean {
  const shape = editor.getShape(shapeId);
  if (!shape) return false;

  const nextOpacity = isContextFrameLayerVisible(shape) ? 0: 1;
  editor.updateShape({
    id: shape.id,
    type: shape.type,
    opacity: nextOpacity,
...
export function deleteContextFrameLayer(editor: Editor, shapeId: TLShapeId, siteId: string): boolean {
  const shape = editor.getShape(shapeId);
  if (!shape) return false;
