...
import { createVoiceReconnectController } from './resilience';
import { registerVoiceKernelImpl } from './voiceKernelImplRegistry';
...
export function useGeminiLive(options: UseGeminiLiveOptions): UseGeminiLiveResult {
  ...
  const pageSessionRef = useRef(ensurePageSession());
  const reconnectRef = useRef(createVoiceReconnectController());
    ...
    if (!clientRef.current) {
      const callbacks = {
        onState: (s: VoiceState) => {
          ...
          if (s === 'connecting') {
            pageSession.setConnectionState('connecting');
          } else if (s === 'listening' || s === 'speaking') {
            pageSession.setConnectionState('connected');
          } else if (s === 'idle') {
            if (!reconnectingRef.current) {
              pageSession.setConnectionState('idle');
            }
          } else if (s === 'error') {
          ...
          if (s === 'listening' && sessionStartRef.current === 0) {
            ...
            sessionIdRef.current = reconnectRef.current.logicalSessionId;
            pageSession.setVoiceSessionId(sessionIdRef.current);
            dispatch('landi:voice-started', { sessionId: sessionIdRef.current });
          }
          if (s === 'idle' && sessionStartRef.current > 0 && !reconnectingRef.current) {
            ...
            pageSession.setVoiceSessionId(null);
          }
        },
        onError: (e: Error) => {
          setError(e.message);
          ...
          if (userStoppedRef.current) {
            pageSessionRef.current.setConnectionState('idle');
            return;
          }
          reconnectingRef.current = true;
          pageSessionRef.current.setConnectionState('reconnecting');
          clientRef.current = null;
          const scheduled = reconnectRef.current.scheduleReconnect(async () => {
            try {
              pageSessionRef.current.setConnectionState('connecting');
              ...
              await c.start();
              reconnectRef.current.resetAttempts();
              reconnectingRef.current = false;
              pageSessionRef.current.setConnectionState('connected');
        ...
        onTranscript: (text: string, role: 'user' | 'assistant') => {
          ...
          pageSessionRef.current.publishTranscript({
            role,
            text,
            timestamp,
            source: 'voice',
      ...
      clientRef.current = useMock
        ? createMockVoiceClient(personaRef.current, callbacks, {
            ...
            simulateDropAfterMs: options.mockSimulateDropAfterMs,
          })
        : createVoiceClient(apiKeySource, personaRef.current, callbacks);
    }
    return clientRef.current;
  }, [apiKey, tokenEndpoint, available, useMock, options.mockScenario, options.mockSimulateDropAfterMs]);
  ...
  const start = useCallback(async () => {
    ...
    reconnectRef.current.cancelPending();
    setError(null);
  ...
  const mountIdRef = useRef<symbol>(Symbol('useGeminiLive'));
  startRef.current = start;
  stopRef.current = stop;
  useEffect(() => {
    return registerVoiceKernelImpl(mountIdRef.current, {
