...
import { resolveConnectGreeting, type VoiceGreetingMode } from './greetingMode';
...
/**
 * Stable acting-agent identity for voice-driven tool calls. Parity with
 * `CHAT_AGENT_TOOL_CONTEXT` in `geminiChatClient.ts`: same binding
 * mechanism, separate identity so provenance can distinguish the modality
 * that drew a mark.
 */
export const VOICE_AGENT_TOOL_CONTEXT: AgentToolExecutionContext = {
  agentId: 'agentable-voice-agent',
...
export function createVoiceClient(
  ...
  async function handleServerMessage(msg: LiveServerMessage): Promise<void> {
    try {
      ...
      //
      // Awaited sequentially, one call at a time, rather than firing every
      // call in the batch concurrently. `withAgentToolContextAsync` binds a
      // module-level context for the duration of its callback and restores
      // the previous value when that callback settles; running several
      // unawaited calls at once would let a faster later call's restore
      // clear the context out from under a still-in-flight earlier call.
      // It happened to work today only because every current handler
      // resolves before that could matter - sequencing removes the
      ...
      for (const call of toolCalls) {
        ...
        const id = call.id;
        // Bind the acting-agent context for the duration of the call, see
        // the matching comment in `geminiChatClient.ts`. Without this,
