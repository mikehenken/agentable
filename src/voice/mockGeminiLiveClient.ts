...
import { resolveConnectGreeting } from './greetingMode';
...
export function createMockVoiceClient(
  ...
  function simulateTransportDrop(): void {
    if (!active || dropSimulated) return;
    dropSimulated = true;
    clearTimers();
    mode = 'idle';
    inputLevel = 0;
    outputLevel = 0;
    active = false;
    callbacks.onError?.(new Error('[mockVoice] transport dropped'));
  ...
  async function start(): Promise<void> {
    ...
    const dropAfter = options.simulateDropAfterMs;
    if (typeof dropAfter === 'number' && dropAfter > 0 && !dropScheduled) {
      dropScheduled = true;
    ...
    if (resumeTurnIndex > 0) {
      runListenThenSpeak(resumeTurnIndex);
      return;
    }
