/**
 * Operator composer mic — spawns Gemini Live voice agent session ( iter-6).
...
export function OperatorVoiceInput({
  ...
  const ariaLabel = voiceActive
    ? defaultVoiceLabel(voice().state, 'End voice session'): dictating
      ? 'Stop dictation': 'Start operator voice agent';
  ...
  return (
    <button
      ...
      title={
        dictationSupported
          ? 'Click: voice agent · Right-click: dictation': 'Start operator voice agent'
      }
