/**
 * Resolve voice persona for operator-surface Gemini Live mount ( iter-6).
...
export function resolveOperatorVoicePersona(): VoicePersonaConfig {
  ...
  const systemPrompt =
    attrPrompt.length > 0
      ? `${DEFAULT_OPERATOR_VOICE_PROMPT}\n\n${attrPrompt}`: creds().systemInstruction.length > 0
        ? `${DEFAULT_OPERATOR_VOICE_PROMPT}\n\n${creds().systemInstruction}`: DEFAULT_OPERATOR_VOICE_PROMPT;
